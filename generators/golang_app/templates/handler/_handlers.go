...
import (
	...
	pb "github.com/yueguanyu/language_master_schedule/proto/schedule"
	"github.com/yueguanyu/language_master_schedule/schedule"
)
...
func sendAllCrons(server pb.ScheduleService_ListScheduleServer, jobs map[cron.EntryID]schedule.CronModel) (err error) {
	for id, model := range jobs {
		sc := pb.Schedule{}
		err = json.Unmarshal([]byte(model.Schedule), &sc)
		if err != nil {
...
func (s *Server) ListSchedule(req *pb.ListScheduleRequest, server pb.ScheduleService_ListScheduleServer) (err error) {
	...
	default:
		for _, jobs := range []map[cron.EntryID]schedule.CronModel{schedule.CronJobs, schedule.OneTimeJobs} {
...
func (s *Server) CreateOneTimeGrpcSchedule(ctx context.Context, req *pb.CreateGrpcScheduleRequest) (*pb.Schedule, error) {
	sd := req.GetSchedule()
	id, err := AddGrpcCron(sd.GetPeriod(), req.GetGrpcCall(), sd, true)
	if err != nil {
...
func (s *Server) CreateGrpcSchedule(ctx context.Context, req *pb.CreateGrpcScheduleRequest) (*pb.Schedule, error) {
	sd := req.GetSchedule()
	id, err := AddGrpcCron(sd.GetPeriod(), req.GetGrpcCall(), sd, false)
	if err != nil {
		log.Println("grpc cron job register fail")
		return nil, err
	}
...
func (s *Server) CreateOneTimeBrokerSchedule(ctx context.Context, req *pb.CreateBrokerScheduleRequest) (*pb.Schedule, error) {
	sd := req.GetSchedule()
	id, err := AddBrokerCron(sd.GetPeriod(), req.GetBrokerEvent(), sd, true)
	if err != nil {
		log.Println("grpc cron job register fail")
		return nil, err
	}
...
func (s *Server) CreateBrokerSchedule(ctx context.Context, req *pb.CreateBrokerScheduleRequest) (*pb.Schedule, error) {
	sd := req.GetSchedule()
	id, err := AddBrokerCron(sd.GetPeriod(), req.GetBrokerEvent(), sd, false)
	if err != nil {
